...
#---------------------------------------------------
impala <- function(data_file,data_type){

#------安装R包--------------------------------------
installed_packs <- installed.packages()[,1]
packs <- c("openxlsx","dplyr","purrr","stringr","forcats","ggplot2")
...
if(length(not_bio)>0){
  ...
  sapply(not_bio,bio_fun)
}

lib_fun <- function(i){library(i,character.only = T)}
sapply(bio_pack,lib_fun)

#------建文件夹----------------------------------
dir_imp <- paste0("impala_analysis_results_",Sys.Date())
if(!dir.exists(dir_imp))
  dir.create(dir_imp)
...
#-----判断数据类型是否正确--------------------
is_gen <- any(grepl("gene",colnames(df),ignore.case = T))

if (is_gen & data_type == "metabolite")
  stop("The data type is error. Pleace check it.")


 no_gen <- !any(grepl("gene",colnames(df),ignore.case = T))
 type <- data_type == "gene-metabolite" | data_type == "protein-metabolites"
...
kegg_pathways$pathway_name <- sub("- .*$","",kegg_pathways$pathway_name)
...
write.xlsx(kegg_pathways,paste0(dir_imp,"/",file.txt,".xlsx"))
...
ggsave(paste0(dir_imp,"/",file.txt,".png"),kegg_pathways_plot,width=1200, height =1000, dpi=150,units = "px")

print( paste0("The metabolite KEGG pathways cound be found in the folder of '",dir_imp,"'"))
