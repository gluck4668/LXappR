...
file_df <- paste0(image_dir,"/",gsub(".*/", "", pic_dir),".xlsx")
write.xlsx(df, file_df)

if(!is.null(hmdb_id_name)){
id_name <- read.xlsx(hmdb_id_name)
df_save <- left_join(df,id_name,by="name")
nn <- ncol(df_save)
df_save <- df_save[,c(nn,1:(nn-1))]
file_save <- paste0(image_dir,"/",gsub(".*/", "", pic_dir),"_id.xlsx")
