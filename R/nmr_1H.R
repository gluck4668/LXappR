...
nmr_1H <- function(test_file=test_file,lib_file=lib_file,lib_select,se=se,data_join){
...
for (t in 1:nrow(test)) {
  ...
  foreach(lib_n=c(1:length(lib_list)),.errorhandling="pass") %do% {
   ...
   result_df <- data.frame()
    ...
    for (x in 1:nrow(lib_df)) {
     ...
     # 获取每一个匹配的数据
     if(all(matches)){
       val <- lapply(lib_x, function(x) test_t[which.min(abs(x - test_t))]) %>%
  ...
  file_save <- paste0(dir_iden,'/',test[t,1],'_(',lib_name[lib_n],')_results_',Sys.Date(),".xlsx")
...
if(length(f_n)>1){
 ...
 d_com <- eval(str2expression(paste0('distinct(d_com,',data_join,',.keep_all = T)')))
...
if(data_join == "id"){
  ...
  # 初始化交集为第一个向量
  com_id <- id_com_list[[1]]

  # 逐个向量求交集
  for (id in id_com_list[-1]) {
    com_id <- intersect(com_id, id)
  }
  ...
  if(length(com_id)>0) {
    ...
    for(x in 1:length(df_com_list)){
      dd <- df_com_list[[x]]
      dd <- dd[dd$id %in% com_id,]
      save_name <- str_extract(names(df_com_list)[x],"(?<=\\/).*")
      write.xlsx(dd, paste0(dir_save,"/",save_name,"_", Sys.Date(),".xlsx"))
    }
  }
  ...
  for (nam in name_com_list[-1]) {
    com_nam <- intersect(com_name, nam)
  }
